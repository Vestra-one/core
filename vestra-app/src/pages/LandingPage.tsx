      ...
      <main className="flex-1">
        <section className="relative py-20 lg:py-32 overflow-hidden">
          <div
            ...
            style={{
              background: 'radial-gradient(circle at 50% 50%, rgba(91, 43, 238, 0.15) 0%, rgba(10, 8, 18, 0) 70%)',
            }}
          />
                ...
                <div className="aspect-square w-full max-w-lg mx-auto rounded-xl border border-[var(--color-border-dark)] bg-[var(--color-card-dark)] p-8 shadow-2xl">
                  <div className="flex h-full flex-col justify-between">
                    <div className="flex items-center justify-between border-b border-[var(--color-border-dark)] pb-4">
                      <span className="text-sm font-bold uppercase tracking-widest text-slate-500">Transaction Hub</span>
                      ...
                      <div className="flex items-center justify-between rounded-lg bg-[var(--color-background-dark)] p-4 border border-[var(--color-border-dark)]">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-full bg-white/10 flex items-center justify-center">
                            <Icon name="account_balance" className="text-[var(--color-primary)]" size={24} />
                          </div>
                          <div>
                            <p className="text-xs text-slate-500">Source Account</p>
                            <p className="text-sm font-bold italic">near.payflow.x</p>
                          </div>
                        </div>
                        <p className="text-sm font-bold text-green-400">10,450 USDC</p>
                      </div>
                      <div className="flex justify-center py-2">
                        <Icon name="arrow_downward" className="text-[var(--color-primary)]" size={24} />
                      </div>
                      <div className="rounded-lg bg-[var(--color-background-dark)] p-4 border border-[var(--color-border-dark)]">
                        <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Destination Address</p>
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-medium">Optimism:0x00...A7b</span>
                          <span className="text-xs font-semibold text-[var(--color-primary)]">MAX</span>
                        </div>
                      </div>
                    </div>
        ...
        <section className="bg-[var(--color-background-dark)] py-24 border-y border-[var(--color-border-dark)]">
          <div className="mx-auto max-w-7xl px-6 lg:px-20">
            ...
            <div className="mx-auto max-w-4xl grid grid-cols-[64px_1fr] gap-x-8">
              {[
                { icon: 'input', title: 'Deposit USDC on NEAR', desc: "Securely fund your master payroll account. NEAR's low latency ensures your funds are ready for distribution in seconds." },
                { icon: 'route', title: 'Intent-Based Routing', desc: "Our smart pathfinding engine automatically selects the fastest and cheapest route to reach your contributors' preferred chains." },
                { icon: 'send_and_archive', title: 'Multi-Chain Settlement', desc: 'Funds arrive simultaneously on Ethereum, Arbitrum, Base, or Optimism. One dashboard to track every penny.' },
              ].map((step, i) => (
        ...
        <section className="py-24">
          <div className="mx-auto max-w-7xl px-6 lg:px-20">
            <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
              ...
              <a className="group flex items-center gap-2 text-[var(--color-primary)] font-bold" href="#">
                View all features <Icon name="arrow_forward" className="group-hover:translate-x-1 transition-transform" size={24} />
              </a>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { icon: 'schedule_send', title: 'Zero-Click Payouts', desc: 'Set it and forget it with smart contracts that handle recurring distributions, vesting schedules, and bonuses automatically.' },
                { icon: 'groups', title: 'Bulk Payments', desc: 'Pay 100+ contributors across different chains in a single transaction. Drastically reduce gas overhead and management time.' },
                { icon: 'description', title: 'Invoice Auto-pay', desc: 'Native integration with on-chain invoicing tools. Verify, approve, and settle invoices with automated reconciliation.' },
              ].map((card) => (
                <div
                  key={card.title}
        ...
        <section className="py-24 px-6">
          <div className="mx-auto max-w-7xl rounded-3xl bg-[var(--color-primary)] px-6 py-16 text-center lg:px-20 lg:py-24 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-3xl font-black text-white sm:text-5xl">Ready to automate your payroll?</h2>
              <p className="mx-auto mt-6 max-w-xl text-lg text-white/80">
                Join the next generation of global organizations using intent-based infrastructure to scale their teams.
              </p>
              <div className="mt-10 flex flex-wrap justify-center gap-4">
                <Link to={ROUTES.dashboard} className="h-14 rounded-full bg-white px-8 text-base font-bold text-[var(--color-primary)] inline-flex items-center">
                  Connect Wallet
                </Link>
                <button type="button" className="h-14 rounded-full border border-white/30 bg-white/10 px-8 text-base font-bold text-white">
                  View Documentation
                </button>
              </div>
            </div>
